...
// 非成员get函数
// 使用C++20的concept约束的通用get函数
template<std::size_t I, typename T>
concept HasGet = requires(T&& t) {
    { std::forward<T>(t).template get<I>() } -> std::same_as<decltype(std::forward<T>(t).template get<I>())>;
};
...
//cl /EHsc /std:c++20 03TupleLikeInheritanceCPP20.cpp /link psapi.lib 