...
struct D:C{
	...
	void print(){
		C::print();
		std::cout<<"d1.size:"<<d1.size()<<"	d1.d2:"<<d2<<'\n';
	}
};
...
int main() {
    ...
    //单链初始化 过程
    {
		std::cout<<"单继承链的初始化过程C c1{{{1.0f},6.11},\"H\",\"M\",\"D\"}\n";
		C c1{{{1.0f},6.11},"H","M","D"};
		c1.print();
		D d1{c1};//用父对象初始化子对象的形式。
		d1.print();
		D d2{1.f,2.f,6.11,7.11,"H","2","3"};//顺序初始化
		d2.print();
    }
    //D的初始化丰富的过程
    {
		//完全过程，用括号标注每个类型的，按顺序，嵌套,MSVC要求比较严格
		D d1{
			{  // 初始化C
				{  // 初始化B
					{1.5f, 2.5f},  // 初始化A
		...
		d1.print();
		 ////部分，花括号里面按顺序，未设置为默认
		//D d2{
		//	{2.0f},                 // A: a1=2.0f, a2=1.0f(默认)
		//	{3.0},                  // B: b1=3.0, b2=0.0(默认)
		//	{"Test"},               // C: c1="Test", c2="", c3=""(默认)
		//	{5, 6, 7},              // D: d1={5,6,7}
		////	 d2 使用默认值 "!D!"
		//};
		//d2.print();
